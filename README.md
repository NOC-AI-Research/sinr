...
## NOC Notes
visualize by running `viz_map.py` as above. I would run `python viz_map.py --taxa_id taxa_id --model_path /path/to/model.pt` where `taxa_id` is the ID on OBIS (so puffinus puffinus would be 137203). 
